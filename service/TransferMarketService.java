...
public class TransferMarketService {
    ...
    public static List<TransferOffer> gerarPropostas(Player jogador) {
        ...
        // Garante clubes diferentes e diferente do clube atual
        List<String> clubesUsados = new ArrayList<>();
        clubesUsados.add(jogador.getClubeAtual().getNome());

        int tentativas = 0;
        while (propostas.size() < quantidade && tentativas < 20) {
            tentativas++;
            Club clube = sortearClube(score, nivelAtual);
            if (clubesUsados.contains(clube.getNome())) continue;
            clubesUsados.add(clube.getNome());
    ...
    private static Club sortearClube(int score, int nivelAtual) {
        ...
        int nivelSorteado = nivelAtual + r.nextInt(nivelMaximo - nivelAtual + 1);
